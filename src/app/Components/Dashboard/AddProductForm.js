      ...
      {message && (
        <div
          className={`mb-4 p-3 rounded ${
            message.type === 'success' ? 'bg-gray-800 text-gray-200' : 'bg-gray-800 text-gray-400'
          }`}
        >
          {message.text}
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4" encType="multipart/form-data">
        <div>
          ...
          <input
            ...
            multiple
            className="w-full p-3 bg-black border border-gray-800 rounded text-white placeholder-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-200/30"
            disabled={loading}
          />
        </div>
        <div>
        ...
        <button
          type="submit"
          className={`w-full p-3 rounded font-medium text-white transition-all duration-300 ${
            loading ? 'bg-gray-800 cursor-not-allowed' : 'bg-gradient-to-br from-gray-900 to-black hover:shadow-[0_0_10px_rgba(255,255,255,0.1)] hover:shadow-lg hover:-translate-y-1'
          }`}
          disabled={loading}
        >
